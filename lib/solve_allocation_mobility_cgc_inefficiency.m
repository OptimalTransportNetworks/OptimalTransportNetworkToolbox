...
-----------------------------------------------------------------------------------
%}
function [results,flag,x]=solve_allocation_mobility_cgc_inefficiency(x0,auxdata,verbose)
...
cost_direct=Apos*(sum(repmat(param.m',[graph.ndeg 1]).*Qin_direct.^param.nu,2).^((param.beta+1)/param.nu).*graph.delta_tau_spillover_ex_direct./kappa_ex);
cost_indirect=Aneg*(sum(repmat(param.m',[graph.ndeg 1]).*Qin_indirect.^param.nu,2).^((param.beta+1)/param.nu).*graph.delta_tau_spillover_ex_indirect./kappa_ex);
...
costpos=sum(matm.*Qin_direct.^param.nu,2).^((param.beta+1)/param.nu-1).*graph.delta_tau_spillover_ex_direct./kappa_ex;
costneg=sum(matm.*Qin_indirect.^param.nu,2).^((param.beta+1)/param.nu-1).*graph.delta_tau_spillover_ex_indirect./kappa_ex; % kappa(j,k) = kappa(k,j) by symmetry
...
if param.nu>1 % if nu=1, diagonal term disappears    
    matpos=repmat( (1+param.beta)*(param.nu-1)*(Apos'*lambda).*costpos.^((param.beta+1)/param.nu-1).*graph.delta_tau_spillover_ex_direct./kappa_ex, [1 param.N]).* repmat(param.m',[graph.ndeg 1]).* Qin_direct.^(param.nu-2);
    matneg=repmat( (1+param.beta)*(param.nu-1)*(Aneg'*lambda).*costneg.^((param.beta+1)/param.nu-1).*graph.delta_tau_spillover_ex_indirect./kappa_ex, [1 param.N]).* repmat(param.m',[graph.ndeg 1]).* Qin_indirect.^(param.nu-2);
...
for n=1:param.N % row
    for m=1:param.N % col
        ...
        vecpos=(1+param.beta)*((1+param.beta)/param.nu-1)*param.nu*...
            (Apos'*lambda).*costpos.^((param.beta+1)/param.nu-2).*graph.delta_tau_spillover_ex_direct./kappa_ex.*...
            param.m(n).*Qin_direct(:,n).^(param.nu-1).*...
            param.m(m).*Qin_direct(:,m).^(param.nu-1);
        
        vecneg=(1+param.beta)*((1+param.beta)/param.nu-1)*param.nu*...
            (Aneg'*lambda).*costneg.^((param.beta+1)/param.nu-2).*graph.delta_tau_spillover_ex_indirect./kappa_ex.*...
